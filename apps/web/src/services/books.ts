...
function mapBook(snap: QueryDocumentSnapshot<DocumentData>): Book {
...
export function subscribeToBooks(userId: string, callback: (books: Book[]) => void) {
  const ownerQ = query(
    collection(db, 'books'),
